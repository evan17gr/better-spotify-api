...
import { checkUserExists } from '../../helpers/reusableQueries.js';
  ...
  try {
    ...
    if (!state || storedState !== state) {
      ...
      if (data) {
        ...
        checkUserExists(profileData, res);
        ...
        res
          .status(200)
          .redirect(
            `http://localhost:3000/profile/${profileData.id}?access_token=${data.access_token}`,
          );
      } else {
        return res.status(401).redirect('http://localhost:3000');
      }
